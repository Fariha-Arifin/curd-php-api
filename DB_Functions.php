...
public function update_data($User_ID,$FirstName,$LastName,$Email,$MobileNumber,$Password){

$stmt= $this->conn->prepare("update user_info set FirstName=?,LastName=?,Email=?,MobileNumber=?,Password=? where User_ID = ?");
$stmt->bind_param("sssssd",$FirstName,$LastName,$Email,$MobileNumber,$Password,$User_ID);
$result=$stmt->execute();
$stmt->close();
